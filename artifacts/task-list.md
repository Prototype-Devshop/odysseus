...
| T4.6 | S4B | Gatekeeper review (workflow/prompt injection, path/secret exposure) | Gatekeeper/Forge | done | Review: `artifacts/gatekeeper-review-media-generation.md`. Remediation landed: **F1** endpoint URL/raw-error stripped from agent-visible text (kept in logs + structured `endpoint` field); **F2** local-by-default guard (`is_local_endpoint` + `allow_remote_media_providers` default false; `remote_blocked` state, URL-free); **F3** MCP owner/session limitation documented (direct chat path stays owner-scoped); **F4** `prompt_id` URL-encoded; **F5** no raw `str(e)` in agent errors; **F8** docstring updated + checkpoint caveat. Tests added in `tests/test_comfyui_provider.py` + `tests/test_generate_image_media.py` |
