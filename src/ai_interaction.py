...
async def do_list_media_models(content: str, session_id: Optional[str] = None, owner: Optional[str] = None) -> Dict:
    ...
    Returns enabled models projected through media_registry.to_public_dict, so
    the response never leaks endpointUrl or workflowPath. When nothing is
    configured, returns the shared degraded-state message from media_registry
    (the actionable "no image model configured" guidance) instead of a vague
    failure.
    """
    ...
    try:
        ...
        for p in public:
            ...
            lines.append(f"- {p.get('label')} (`{p.get('id')}`, {p.get('provider')}){tag} — {caps}")
        if not default_id:
            lines.append("")
        ...
        logger.error(f"list_media_models failed: {e}")
        return {"error": str(e)}
...
async def dispatch_ai_tool(
    ...
    elif tool == "list_media_models":
        kind = content.strip()[:20]
        desc = f"list_media_models{': ' + kind if kind else ''}"
        result = await do_list_media_models(content, session_id, owner=owner)
