...
async def test_returns_enabled_models_without_leaking_paths(monkeypatch):
    ...
    result = await ai_interaction.do_list_media_models("", owner="alice")
    ...
    # Default flag reflects the resolved default.
...
async def test_returns_enabled_models_text_has_no_url(monkeypatch):
    # The human-readable results string must also be path/URL free.
...
async def test_dispatch_routes_and_passes_owner(monkeypatch):
    seen = {}
    ...
    monkeypatch.setattr(ai_interaction, "do_list_media_models", fake_impl)

    desc, result = await ai_interaction.dispatch_ai_tool(
        "list_media_models", "image", session_id="sid1", owner="alice"
    )
    ...
    assert "list_media_models" in desc
